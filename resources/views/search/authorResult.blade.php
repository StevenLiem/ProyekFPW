...
@section("mainContent")
    <div class="text-center fs-2 my-3">
        Author <span class="badge bg-secondary">{{ $author->name }}
            <span class="badge bg-dark px-2">{{ count($author->mangas) }}</span>
        </span>
    </div>
    <div class="container rounded" style="background-color: #191a1c;">
        <div class="d-flex flex-wrap justify-content-center m-3">
            @if (count($author->mangas) == 0)
                <h3 class="p-3">No results, sorry</h3>
            @endif
            @foreach ($author->mangas as $manga)
                @php
